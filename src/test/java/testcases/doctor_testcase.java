...
public class doctor_testcase extends AppTestBase {
	...
	public void takingScreenshotOfCurrentPage() throws Exception {
		doctor_PagesInstance = new doctor_Pages(driver);
		locatorsFactoryInstance = new LocatorsFactory(driver);
		Assert.assertTrue(doctor_PagesInstance.takingScreenshotOfTheCurrentPage(),
				"Not able to take the screenshot, please check manually");
		Assert.assertTrue(locatorsFactoryInstance.verifyOutPatientMenuIsPresent(driver).isDisplayed(),
				"OutPatient Menu is not present in the current page, Please check manually");
	}
	...
	public void performKeyboardOperation() throws Exception {
		...
		Map<String, String> expectedData = new FileOperations().readExcelPOI(expectedDataFilePath, "DoctotModule");
		Assert.assertEquals(doctor_PagesInstance.performKeyboardOperation(),
				expectedData.get("titleNameOfVaccinationPatientRegisterForm"),
				"Error in Page class something went wroung, please check manually");
		Assert.assertEquals(locatorsFactoryInstance.verifyTitleNameOfVaccinationPatientRegisterFormIsPresent(),
				expectedData.get("titleNameOfVaccinationPatientRegisterForm"),
				"title Name Of Vaccination Patient Register Form is not matching in current page(locators factory), Please check manually");
	}
	...
	public void verifyTooltipOfAnElement() throws Exception {
		...
		Assert.assertEquals(doctor_PagesInstance.verifyToolTipOfAnElement(), expectedData.get("newPatientTooltipValue"),
		...
		Assert.assertTrue(locatorsFactoryInstance.tooltipTextIsPresent(driver).isDisplayed(),
				"Tooltip is not present in the current page, Please check manually");
	}
	...
	public void handleAlertPopup() throws Exception {
		...
		locatorsFactoryInstance = new LocatorsFactory(driver);
		Map<String, String> expectedData = new FileOperations().readExcelPOI(expectedDataFilePath, "DoctotModule");
		Assert.assertEquals(doctor_PagesInstance.handleAlertPopup(expectedData),
				expectedData.get("membershipDropdownOption"), "Verification failed, please check manually");
		Assert.assertEquals(locatorsFactoryInstance.verifyNhifCapitationIsSelected(),
				expectedData.get("membershipDropdownOption"),
				"selected option is not matching with expected, please check manually!");
	}
	...
	public void handleAutoSuggesstion() throws Exception {
		...
		Assert.assertTrue(doctor_PagesInstance.handleAutoSuggesstion(expectedData),
				"User is not abale to handle the search bar, please check manually");
		Assert.assertTrue(locatorsFactoryInstance.accountOptionIsSelected(driver).isDisplayed(),
				"Account option is not selected, Please check manually");
	}
